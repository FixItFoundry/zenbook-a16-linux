...
Full log: `logs/edp-hbr3-RESULT.log`, `logs/edp-hbr3-bind.log`.
...
**This was our DTB's bug, not upstream's.** The fix (`dts/test62.dts`, exactly one
...
- `dts/test62.dts` already permits it: the eDP endpoint's `link-frequencies` last
...
| SMMU faults, missing panel node, gpucc, TrustZone XPU wall | All closed earlier; see `HANDOFF.md` |
...
**Closed 2026-07-24 20:28.** `logs/edp-hbr3-stockphy-RESULT.log`.
...
| File | What |
|---|---|
| `logs/edp-hbr3-stockphy-RESULT.log` | **the confirmation run** — stock swing table, trains at `0x11`/`0x15` |
| `logs/edp-hbr3-RESULT.log` | **the link-up** — verdict, PHY returns, sink caps, decoded link status |
| `logs/edp-hbr3-bind.log` | full unbuffered dmesg capture of that bind |
| `logs/edp-rateset3-*.log` | Bug 2 fix verified working, EQ still fails at 5.4 G |
| `logs/edp-rateset2-*.log` | the run that exposed Bug 2 (`use_rate_set=1`, no `0x115` write) |
| `logs/edp-phywin-*.log` | PHY register-window dump — exonerates the PHY end to end |
| `logs/edp-muxen-*.log` | MUX_EN hypothesis killed |
| `logs/edp-test62-RESULT.log` | 5.4 G swing/emphasis proven inert |
| `docs/edp-firmware-re-2026-07-24.md` | UEFI DisplayDxe reverse-engineering write-up |
| `patches/glymur-edp-hbr3-EXPERIMENT.patch` | the HBR3 force |
| `patches/glymur-edp-rate-set-EXPERIMENT.patch` | the Bug 2 rate-set plumbing fix |

Reproduction: **`docs/edp-enable-howto.md`**.
