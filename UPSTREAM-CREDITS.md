...
|---|---|---|---|
| **ASUS Zenbook A16 (UX3607OA) keyboard support** — I2C-HID `0B05:4B42`, vendor usages 0x85 → `KEY_CAMERA`, 0x86 → `KEY_PROG1`, 0x5f → `KEY_PROG2`, and `QUIRK_FILTER_CAMERA_COMPANION` for the camera-toggle companion byte that otherwise dims the panel | **Konrad Dybcio** \<konrad.dybcio@oss.qualcomm.com\> | `HID: asus: support the Zenbook A16 (UX3607OA) keyboard`, 2026-07-24, msg-id `20260724-topic-asus_keyboard-v1-1-a746ff8f77b2@oss.qualcomm.com` | **Adopted 2026-07-27, but NOT verbatim — corrected 2026-07-31.** It replaced our own earlier hand-rolled mapping, which was wrong — we had 0x85 as `KEY_KBDILLUMTOGGLE` and it never worked on the hardware. ⚠️ **This entry previously claimed "adopted verbatim" and that was inaccurate.** One line of Konrad's mapping did not survive: `0x5f` shipped as `KEY_PROG1` in `patches/glymur-hid-asus-a16-7.2.patch`, a leftover from our hand-rolled version, where Konrad has **`KEY_PROG2`**. Both the MyASUS key (`0x86`) and the S-shaped programmable key (`0x5f`) therefore emitted the same `XF86Launch1`, and no userspace could tell them apart. Fixed on 2026-07-31 to match Konrad (`hid-asus.c:1229`, module srcversion `25FED7161F4D70E10A46EB0`). **Konrad's mapping was right and ours was the deviation.** The rest of the mapping is confirmed correct on this machine. |
...
| **eDP v8 PHY power-on programming sequence fix** — `phy: qcom: edp: split power-on sequencing by PHY version` + `phy: qcom: edp: update v8 power-on programming sequence`, `drivers/phy/qualcomm/phy-qcom-edp.c` | **Bjorn Andersson** \<bjorn.andersson@oss.qualcomm.com\> | `[PATCH 0/2] phy: qcom: edp: Update v8 programming sequence`, 2026-06-22, msg-id `20260622-glymur-edp-phy-v1-0-814b45089ac9@oss.qualcomm.com`, **still unmerged as of `next-20260819`** | **Adopted verbatim 2026-08-19** as `patches/next-20260817/0014` and `0015`. This is the real fix for the HBR3-only eDP training bug we had reported and were working around locally by forcing the link above the panel's advertised maximum (`0004`, now reverted as `0016`). Validated on hardware: with this series applied, the panel trains natively at HBR2 (its actual advertised maximum) instead of needing to be forced to HBR3. Konrad Dybcio pointed us to this series in his reply on our eDP bug report. |
